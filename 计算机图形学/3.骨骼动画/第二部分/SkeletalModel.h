...
public:
	...
	// 递归绘制关节
	void drawOneJoint(Joint* joint);

	// 递归绘制骨骼
	void drawBouns(Joint* joint);
	
	// 绘制关节间的骨骼
	void drawOneBouns(Joint* son);

	// 初始化绑定矩阵
	void computeOneJoint(Joint* joint, MatrixStack& ms);

	// 递归更新 SSD 矩阵
	void updateOneJoint(Joint* joint, MatrixStack& ms);
